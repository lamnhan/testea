...
export class RenderService {
  ...
  async render(parsedItem: ParsedItem) {
    ...
    // main imports
    const relPathSegs = (path.replace(/\\/g, '/').split('/src/').pop() as string)
      .split('/');
    ...
    classes.forEach(cls => {
      ...
      result.push(
        '',
        this.templateService.getSetup(path, cls, moduleImports)
      );
      // suite
      ...
      const methodCases = methods.map(meth =>
        this.templateService.getCaseForMethod(meth.NAME, meth.PARAMETERS, cls.name));
